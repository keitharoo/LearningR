...
amzn <- window(AMZN['2014-10-01::2015-09-30'])
...
sma5 <- SMA(Cl(amzn),n=5)
sma21 <- SMA(Cl(amzn),n=21)
# Technical Analysis Chart
# barChart(amzn)
# addSMA(n=5,col=4)
# addSMA(n=21,col=6)
# Manual Chart
# plot(Cl(amzn),main="Simple Moving Averages SMA(5 & 21)")
# lines(sma5,col=4)
# lines(sma21,col=6)

# Exponential Moving Averages EMA(5 & 21)
ema5 <- EMA(Cl(amzn),n=5)
ema21 <- EMA(Cl(amzn),n=21)
# Technical Analysis Chart
barChart(amzn)
addEMA(n=5,col=4)
addEMA(n=21,col=6)
# Manual Chart
plot(Cl(amzn),main="Exponential Moving Averages EMA(5 & 21)")
lines(ema5,col=4)
lines(ema21,col=6)

# 2.1.2. Bollinger Bands BB(20,2)
bb <- BBands(cbind(Hi(amzn),Lo(amzn),Cl(amzn)),n=20,sd=2)
# Technical Analysis Chart
barChart(amzn)
addBBands(n=20,sd=2)
# Manual Chart
# plot(Cl(amzn),main="Bollinger Bands BB(20,2)")
...
sar <- SAR(cbind(Hi(amzn),Lo(amzn)),accel=c(0.02, 0.2))
# Technical Analysis Chart
# barChart(amzn)
# addSAR(accel=c(0.02, 0.2))
# Manual Chart
# plot(Cl(amzn),main="Parabolic Stop and Reverse SAR(0.02,0.2)")
# points(sar,col=4)
...
adx <- ADX(cbind(Hi(amzn),Lo(amzn),Cl(amzn)),n=14)
# Technical Analysis Chart
# barChart(amzn)
# addADX(n=14)

# 2.2.2. Commodity Channel Index CCI(20,0.015)
cci <- CCI(cbind(Hi(amzn),Lo(amzn),Cl(amzn)),n=20,c=0.015)
# Technical Analysis Chart
# barChart(amzn)
# addCCI(n=20,c=0.015)

# 2.2.3. Moving Averages Covergence/Divergence MACD(12,26,9)
macd <- MACD(Cl(amzn),nFast=12,nSlow=26,nSig=9)
# Technical Analysis Chart
# barChart(amzn)
# addMACD()
...
smi <- SMI(cbind(Hi(amzn),Lo(amzn),Cl(amzn)),n=13,nFast=2,nSlow=25,nSig=9)
# Technical Analysis Chart
# barChart(amzn)
# addSMI(n=13)

# 2.2.7. Williams %R(14)
wpr <- WPR(cbind(Hi(amzn),Lo(amzn),Cl(amzn)),n=14)
# Technical Analysis Chart
# barChart(amzn)
# addWPR(n=14)
...
sma5tr <- Lag(ifelse(Lag(Cl(amzn))<Lag(sma5)&Cl(amzn)>sma5,1,ifelse(Lag(Cl(amzn))>Lag(sma5)&Cl(amzn)<sma5,-1,0)))
sma5tr[is.na(sma5tr)] <- 0
sma21tr <- Lag(ifelse(Lag(Cl(amzn))<Lag(sma21)&Cl(amzn)>sma21,1,ifelse(Lag(Cl(amzn))>Lag(sma21)&Cl(amzn)<sma21,-1,0)))
...
# addEMA(n=21,col=6)
# Price Crossover Trading Signals
ema5tr <- Lag(ifelse(Lag(Cl(amzn))<Lag(ema5)&Cl(amzn)>ema5,1,ifelse(Lag(Cl(amzn))>Lag(ema5)&Cl(amzn)<ema5,-1,0)))
ema5tr[is.na(ema5tr)] <- 0
ema21tr <- Lag(ifelse(Lag(Cl(amzn))<Lag(ema21)&Cl(amzn)>ema21,1,ifelse(Lag(Cl(amzn))>Lag(ema21)&Cl(amzn)<ema21,-1,0)))
...
bbtr <- Lag(ifelse(Lag(Cl(amzn))<Lag(bb[,1])&Cl(amzn)>bb[,1],1,ifelse(Lag(Cl(amzn))<Lag(bb[,3])&Cl(amzn)>bb[,3],-1,0)))
bbtr[is.na(bbtr)] <- 0
...
sartr <- Lag(ifelse(Lag(Cl(amzn))<Lag(sar)&Cl(amzn)>sar,1,ifelse(Lag(Cl(amzn))>Lag(sar)&Cl(amzn)<sar,-1,0)))
sartr[is.na(sartr)] <- 0
...
ccismatr <- Lag(ifelse(Lag(Cl(amzn))<Lag(sma5)&Cl(amzn)>sma5&cci<(-100),1,ifelse(Lag(Cl(amzn))>Lag(sma5)&Cl(amzn)<sma5&cci>100,-1,0)))
...
# addSMA(n=5,col=4)
# Price Crossover and Bands Crossover Confirmation Trading Signals 
rocsmatr <- Lag(ifelse(Lag(Cl(amzn))<Lag(sma5)&Cl(amzn)>sma5&roc[,4]<(-0.10),1,ifelse(Lag(Cl(amzn))>Lag(sma5)&Cl(amzn)<sma5&roc[,4]>0.10,-1,0)))
...
# addSMA(n=5,col=4)
# Price Crossover and Bands Crossover Confirmation Trading Signals 
rsismatr <- Lag(ifelse(Lag(Cl(amzn))<Lag(sma5)&Cl(amzn)>sma5&rsi<30,1,ifelse(Lag(Cl(amzn))>Lag(sma5)&Cl(amzn)<sma5&rsi>70,-1,0)))
...
# addSMA(n=5,col=4)
# Price Crossover and Bands Crossover Confirmation Trading Signals 
smismatr <- Lag(ifelse(Lag(Cl(amzn))<Lag(sma5)&Cl(amzn)>sma5&smi[,1]<(-40),1,ifelse(Lag(Cl(amzn))>Lag(sma5)&Cl(amzn)<sma5&smi[,1]>40,-1,0)))
...
# addSMA(n=5,col=4)
# Price Crossover and Bands Crossover Confirmation Trading Signals 
wprsmatr <- Lag(ifelse(Lag(Cl(amzn))<Lag(sma5)&Cl(amzn)>sma5&wpr>0.80,1,ifelse(Lag(Cl(amzn))>Lag(sma5)&Cl(amzn)<sma5&wpr<0.20,-1,0)))
...
for(i in 1:length(Cl(amzn))){sma5sig[i] <- ifelse(sma5tr[i]==1,1,ifelse(sma5tr[i]==-1,0,sma5sig[i-1]))}
sma5sig[is.na(sma5sig)] <- 1
sma21sig <- ifelse(sma21tr>1,0,1)
for(i in 1:length(Cl(amzn))){sma21sig[i] <- ifelse(sma21tr[i]==1,1,ifelse(sma21tr[i]==-1,0,sma21sig[i-1]))}
...
for(i in 1:length(Cl(amzn))){ema5sig[i] <- ifelse(ema5tr[i]==1,1,ifelse(ema5tr[i]==-1,0,ema5sig[i-1]))}
ema5sig[is.na(ema5sig)] <- 1
ema21sig <- ifelse(ema21tr>1,0,1)
for(i in 1:length(Cl(amzn))){ema21sig[i] <- ifelse(ema21tr[i]==1,1,ifelse(ema21tr[i]==-1,0,ema21sig[i-1]))}
...
for(i in 1:length(Cl(amzn))){smasig[i] <- ifelse(smatr[i]==1,1,ifelse(smatr[i]==-1,0,smasig[i-1]))}
smasig[is.na(smasig)] <- 1
emasig <- ifelse(ematr>1,0,1)
for(i in 1:length(Cl(amzn))){emasig[i] <- ifelse(ematr[i]==1,1,ifelse(ematr[i]==-1,0,emasig[i-1]))}
emasig[is.na(emasig)] <- 1

# 4.2. Bollinger Bands BB(20,2) Trading Strategy
# Bands Crossover Trading Strategy
bbsig <- ifelse(bbtr>1,0,1)
for(i in 1:length(Cl(amzn))){bbsig[i] <- ifelse(bbtr[i]==1,1,ifelse(bbtr[i]==-1,0,bbsig[i-1]))}
bbsig[is.na(bbsig)] <- 1

# 4.3. Parabolic Stop And Reverse SAR(0.02,0.2) Trading Strategy
# Stop And Reverse Trading Strategy
sarsig <- ifelse(sartr>1,0,1)
for(i in 1:length(Cl(amzn))){sarsig[i] <- ifelse(sartr[i]==1,1,ifelse(sartr[i]==-1,0,sarsig[i-1]))}
...
for(i in 1:length(Cl(amzn))){adxsig[i] <- ifelse(adxtr[i]==1,1,ifelse(adxtr[i]==-1,0,adxsig[i-1]))}
adxsig[is.na(adxsig)] <- 1

# 4.5. Commodity Channel Index CCI(20,0.015) Trading Strategy
# Bands Crossover Trading Strategy
ccisig <- ifelse(ccitr>1,0,1)
for(i in 1:length(Cl(amzn))){ccisig[i] <- ifelse(ccitr[i]==1,1,ifelse(ccitr[i]==-1,0,ccisig[i-1]))}
...
for(i in 1:length(Cl(amzn))){smacdsig[i] <- ifelse(smacdtr[i]==1,1,ifelse(smacdtr[i]==-1,0,smacdsig[i-1]))}
smacdsig[is.na(smacdsig)] <- 1
cmacdsig <- ifelse(cmacdtr>1,0,1)
for(i in 1:length(Cl(amzn))){cmacdsig[i] <- ifelse(cmacdtr[i]==1,1,ifelse(cmacdtr[i]==-1,0,cmacdsig[i-1]))}
cmacdsig[is.na(cmacdsig)] <- 1

# 4.7. Rate Of Change ROC(21) Trading Strategy
# Bands Crossover Trading Strategy
rocsig <- ifelse(roctr>1,0,1)
for(i in 1:length(Cl(amzn))){rocsig[i] <- ifelse(roctr[i]==1,1,ifelse(roctr[i]==-1,0,rocsig[i-1]))}
rocsig[is.na(rocsig)] <- 1

# 4.8. Relative Strength Index RSI(14) Trading Strategy
# Bands Crossover Trading Strategy
rsisig <- ifelse(rsitr>1,0,1)
for(i in 1:length(Cl(amzn))){rsisig[i] <- ifelse(rsitr[i]==1,1,ifelse(rsitr[i]==-1,0,rsisig[i-1]))}
...
for(i in 1:length(Cl(amzn))){smisig[i] <- ifelse(smitr[i]==1,1,ifelse(smitr[i]==-1,0,smisig[i-1]))}
smisig[is.na(smisig)] <- 1

# 4.10. Williams %R(14) Trading Strategy
# Bands Crossover Trading Strategy
wprsig <- ifelse(wprtr>1,0,1)
for(i in 1:length(Cl(amzn))){wprsig[i] <- ifelse(wprtr[i]==1,1,ifelse(wprtr[i]==-1,0,wprsig[i-1]))}
...
for(i in 1:length(Cl(amzn))){ccismasig[i] <- ifelse(ccismatr[i]==1,1,ifelse(ccismatr[i]==-1,0,ccismasig[i-1]))}
...
for(i in 1:length(Cl(amzn))){rocsmasig[i] <- ifelse(rocsmatr[i]==1,1,ifelse(rocsmatr[i]==-1,0,rocsmasig[i-1]))}
...
for(i in 1:length(Cl(amzn))){rsismasig[i] <- ifelse(rsismatr[i]==1,1,ifelse(rsismatr[i]==-1,0,rsismasig[i-1]))}
...
for(i in 1:length(Cl(amzn))){smismasig[i] <- ifelse(smismatr[i]==1,1,ifelse(smismatr[i]==-1,0,smismasig[i-1]))}
...
for(i in 1:length(Cl(amzn))){wprsmasig[i] <- ifelse(wprsmatr[i]==1,1,ifelse(wprsmatr[i]==-1,0,wprsmasig[i-1]))}
